-- Limpiar y estandarizar pagos recibidos
-- Reglas   :
--   1. Excluir REVERSED → no afecta recaudo
--   2. Excluir PENDING  → aún no confirmado
--   3. Excluir monto = 0 (P106)
--   4. Canal NULL → mapeado a 'UNKNOWN' (P102)
--   5. Detectar pagos huérfanos (installment_id no existe) → P101
--   6. Detectar inconsistencia loan/installment (loan del pago ≠ loan de la cuota) → P102, P104
-- Supuesto: solo pagos CONFIRMED con monto > 0 entran a la capa analítica.
--           Los demás se conservan en raw y se registran en la vista de auditoría.
